...
public class ParticipantRestController {
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	 public ResponseEntity<?> removeParticipant(@PathVariable("id") String login){
		Participant removedParticipant = participantService.findByLogin(login);
	     if (removedParticipant == null) {
	    	 return new ResponseEntity(HttpStatus.NOT_FOUND);
	     }
		participantService.remove(removedParticipant);
		return new ResponseEntity<Participant>(removedParticipant, HttpStatus.OK);
